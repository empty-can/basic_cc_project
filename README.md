...
> 配布本体 `basic_dot_claude` → 本リポ）の設計・手順・レビューの完成版ドキュメントは
...
- `CLAUDE.md.sample` … コピー先で `CLAUDE.md` に改名して使うひな型
  （`.sample` のままなら memory として非ロードなので、改名するまで何も起きない）。
  個人用ローカル指示の雛型が要るなら、供給元 `base-dev-kit-for-cc` ルートの
  `CLAUDE.local.md.example` を参照する（本ハブには同梱していない）。
